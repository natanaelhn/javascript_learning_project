...
const { buyTollRoadCard, topUpBalance, useTollRoad } = require('./index8_utils');

async function getTollAccess() {
  try{
    let card = await buyTollRoadCard(25);
    card = await topUpBalance(card, 10);
    await useTollRoad(card);
  }
  catch(error){
